...
hp <- subset(results, results$Proteinas > 0)
max_x <- max(hp$Tiempo, na.rm = TRUE)
max_y <- max(hp$Proteinas, na.rm = TRUE)
#ggplot(hp, aes (x=Tiempo, y=Proteinas))+geom_point()   4*0:g_range[2]
x_range_vector <- 10*0:max_x
g <- ggplot(hp, aes (x=Tiempo, y=Proteinas))+geom_point()+ geom_line(aes(color = Proteinas))
g + scale_x_continuous(name ="Tiempo(hrs)", x_range_vector, x_range_vector, limits=c(0,max_x ))
g + scale_y_discrete(name ="Proteinas(gr/L)", limits=c(0,max_y))
ggsave("Proteinas.png", device = "png", path = imgDir)
